...
namespace UnitySampleAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
		...
		[SerializeField] private float lookSpeedX = 3; //AMCLEAN added lookSpeedx
		[SerializeField] private float lookSpeedY = 1; //AMCLEAN added lookSpeedx
        ...
        private void RotateView()
        {
			...
			Vector2 mouseInput = new Vector2(CrossPlatformInputManager.GetAxisRaw("HorizontalLook"), 
			                                 CrossPlatformInputManager.GetAxisRaw("VerticalLook")); //AMCLEAN add
            // handle the roation round the x axis on the camera

			float camX = _camera.transform.localEulerAngles.x; //AMCLEAN Add
			...
			{
				_camera.transform.localEulerAngles += new Vector3(-mouseInput.y * lookSpeedY, _camera.transform.localEulerAngles.y,
				                                                  _camera.transform.localEulerAngles.z);
			}
