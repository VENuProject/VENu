...
public class twoJoyControl : MonoBehaviour {
	...
	void Update () {
		...
		cameraMount.transform.Rotate(0, CrossPlatformInputManager.GetAxis("HorizontalLook") * 3 * lookSensitivity * horizLookMultiplier, 0);
		float xDelta = CrossPlatformInputManager.GetAxis("VerticalLook") * lookSensitivity * 3;
